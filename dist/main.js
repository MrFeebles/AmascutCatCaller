...
var SHOW_LAST_N = 3;
var HEX = {
    white: "#e6eaf2", yellow: "#ffdd57", red: "#ff5252", green: "#10b981",
    blue: "#3b82f6", gray: "#9ca3af", purple: "#a855f7",
};
var out = function () { return document.getElementById("output"); };
function appendLine(chunks) {
    var _a;
    var el = out();
    if (!el)
        return;
    ...
    for (var _i = 0, arr_1 = arr; _i < arr_1.length; _i++) {
        ...
        if (typeof c === "string")
            ...
            span.style.color = HEX[(_a = c.color) !== null && _a !== void 0 ? _a : "white"];
        }
...
function identifyApp() {
    var _a;
    var paths = ["./appconfig.json", "appconfig.json", "../appconfig.json"];
    for (var _i = 0, paths_1 = paths; _i < paths_1.length; _i++) {
        var p = paths_1[_i];
        try {
            (_a = alt1 === null || alt1 === void 0 ? void 0 : alt1.identifyAppUrl) === null || _a === void 0 ? void 0 : _a.call(alt1, p);
        }
        catch (_b) { }
    }
}
// ---------- TTS ----------
var TTS_VOLUME = (function () {
    var v = Number(localStorage.getItem("ttsVolume"));
    return isNaN(v) ? 1 : Math.max(0, Math.min(1, v));
})();
var TTS_MUTED = ((_a = localStorage.getItem("ttsMuted")) !== null && _a !== void 0 ? _a : "0") === "1";
var TTS_LAST = (function () {
    var v = Number(localStorage.getItem("ttsLastVol"));
    return isNaN(v) ? 1 : Math.max(0, Math.min(1, v));
})();
var ttsVoice = null;
function chooseVoice() {
    ...
    ttsVoice = vs.find(function (v) { return /en[-_](US|GB)/i.test(v.lang); }) || vs.find(function (v) { return /en/i.test(v.lang); }) || vs[0] || null;
}
if ("speechSynthesis" in window) {
    (_c = (_b = window.speechSynthesis).addEventListener) === null || _c === void 0 ? void 0 : _c.call(_b, "voiceschanged", function () { if (!ttsVoice)
        chooseVoice(); });
    chooseVoice();
}
function speak(line) {
    if (!("speechSynthesis" in window))
        return;
    var text = Array.isArray(line)
        ? line.map(function (c) { return typeof c === "string" ? c : c.text; }).join("")
    ...
    u.rate = 1.0;
    u.pitch = 1.0;
    u.volume = TTS_MUTED ? 0 : TTS_VOLUME;
    try {
...
function setMuteUI(muted) {
    TTS_MUTED = muted;
    localStorage.setItem("ttsMuted", muted ? "1" : "0");
    var btn = document.getElementById("btnMute");
    if (btn)
        btn.textContent = muted ? "🔇" : "🔊";
    btn === null || btn === void 0 ? void 0 : btn.classList.toggle("muted", muted);
    var pct = document.getElementById("volPct");
    if (pct)
        pct.textContent = "".concat(Math.round((muted ? 0 : TTS_VOLUME) * 100), "%");
    var slider = document.getElementById("ttsVol");
    if (slider && !muted)
        slider.value = String(Math.round(TTS_VOLUME * 100));
}
...
function setupControlsUI() {
    ...
    setMuteUI(TTS_MUTED);
    if (pct)
        pct.textContent = "".concat(Math.round((TTS_MUTED ? 0 : TTS_VOLUME) * 100), "%");
    if (slider)
        slider.value = String(Math.round(TTS_VOLUME * 100));
    btnBase === null || btnBase === void 0 ? void 0 : btnBase.classList.toggle("active", BASE_ENABLED);
    btnOverlay === null || btnOverlay === void 0 ? void 0 : btnOverlay.addEventListener("click", function () { return btnOverlay.classList.toggle("active"); });
    btnBase === null || btnBase === void 0 ? void 0 : btnBase.addEventListener("click", function () {
        BASE_ENABLED = !BASE_ENABLED;
        localStorage.setItem("baseEnabled", BASE_ENABLED ? "1" : "0");
        btnBase.classList.toggle("active", BASE_ENABLED);
    });
    slider === null || slider === void 0 ? void 0 : slider.addEventListener("input", function () {
        var v = Math.min(100, Math.max(0, Number(slider.value))) / 100;
        TTS_VOLUME = v;
        localStorage.setItem("ttsVolume", String(TTS_VOLUME));
        if (TTS_MUTED && v > 0) {
            TTS_LAST = v;
            localStorage.setItem("ttsLastVol", String(v));
            setMuteUI(false);
        }
        else if (!TTS_MUTED) {
            TTS_LAST = v;
    ...
    btnMute === null || btnMute === void 0 ? void 0 : btnMute.addEventListener("click", function () {
        var _a, _b;
        if (!TTS_MUTED) {
            TTS_LAST = TTS_VOLUME;
            localStorage.setItem("ttsLastVol", String(TTS_LAST));
            setMuteUI(true);
            (_b = (_a = window.speechSynthesis) === null || _a === void 0 ? void 0 : _a.cancel) === null || _b === void 0 ? void 0 : _b.call(_a);
        }
        else {
            TTS_VOLUME = TTS_LAST || 1;
            localStorage.setItem("ttsVolume", String(TTS_VOLUME));
            setMuteUI(false);
        }
    });
...
// ---------- Helpers ----------
function fire(resp) { appendLine(resp); speak(resp); }
...
var seenFIFO = [];
var scheduledPair = new Set();
var SEEN_LIMIT = 4000;
function tsKey(s) { var m = TS_RE.exec(s); return m ? m[0] : "no-ts"; }
function normContent(s) { return s.replace(TS_RE, "").replace(/\s+/g, " ").trim().toLowerCase(); }
function pairKey(raw) { return "".concat(tsKey(raw), "|").concat(normContent(raw)); }
function rememberPair(k) {
    if (!seenPair.has(k)) {
        seenPair.add(k);
        seenFIFO.push(k);
        if (seenFIFO.length > SEEN_LIMIT) {
            var drop = seenFIFO.shift();
            if (drop)
                seenPair.delete(drop);
        }
    }
}
// ---------- Chat reader ----------
...
function initReader() {
    ...
    var add = function (r, g, b, tol, step) {
        var _a, _b;
        if (tol === void 0) { tol = 24; }
        if (step === void 0) { step = 8; }
        var clamp = function (v) { return Math.max(0, Math.min(255, v)); };
        var arr = [];
        for (var dr = -tol; dr <= tol; dr += step)
            for (var dg = -tol; dg <= tol; dg += step)
                for (var db = -tol; db <= tol; db += step)
                    arr.push((_b = ((_a = alt1__WEBPACK_IMPORTED_MODULE_0__ === null || alt1__WEBPACK_IMPORTED_MODULE_0__ === void 0 ? void 0 : alt1__WEBPACK_IMPORTED_MODULE_0__.mixcolor) === null || _a === void 0 ? void 0 : _a.call(alt1__WEBPACK_IMPORTED_MODULE_0__, clamp(r + dr), clamp(g + dg), clamp(b + db)))) !== null && _b !== void 0 ? _b : ((clamp(r + dr) << 16) | (clamp(g + dg) << 8) | (clamp(b + db))));
...
function extractRect(res) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s;
    var r = (_c = (_b = (_a = res === null || res === void 0 ? void 0 : res.mainbox) === null || _a === void 0 ? void 0 : _a.rect) !== null && _b !== void 0 ? _b : res === null || res === void 0 ? void 0 : res.rect) !== null && _c !== void 0 ? _c : (_e = (_d = res === null || res === void 0 ? void 0 : res.boxes) === null || _d === void 0 ? void 0 : _d[0]) === null || _e === void 0 ? void 0 : _e.rect;
    if (!r)
        return null;
    var x = (_g = (_f = r.x) !== null && _f !== void 0 ? _f : r.left) !== null && _g !== void 0 ? _g : 0, y = (_j = (_h = r.y) !== null && _h !== void 0 ? _h : r.top) !== null && _j !== void 0 ? _j : 0;
    var w = (_l = (_k = r.w) !== null && _k !== void 0 ? _k : r.width) !== null && _l !== void 0 ? _l : (((_m = r.right) !== null && _m !== void 0 ? _m : 0) - ((_o = r.left) !== null && _o !== void 0 ? _o : 0));
...
function tick() {
    ...
    var _loop_1 = function (l) {
        ...
        if (seenPair.has(key))
            return "continue";
        var _loop_2 = function (mech) {
            if (!mech.trigger.test(raw))
                return "continue";
            var run = function () {
                ...
                if (mech.response) {
                    ...
                    if (mech.repeat && mech.repeat > 0)
                        setTimeout(function () { return fire(mech.response); }, mech.repeat);
                }
            };
            var d = (_a = mech.delay) !== null && _a !== void 0 ? _a : 0;
            if (d > 0) {
                if (!scheduledPair.has(key)) {
                    scheduledPair.add(key);
                    setTimeout(run, d);
                }
            }
            else {
        ...
        for (var _d = 0, ACTIVE_1 = ACTIVE; _d < ACTIVE_1.length; _d++) {
            var mech = ACTIVE_1[_d];
            var state_1 = _loop_2(mech);
            if (state_1 === "break")
                break;
        }
    };
    for (var _i = 0, prim_1 = prim; _i < prim_1.length; _i++) {
        var l = prim_1[_i];
        _loop_1(l);
    }
}
